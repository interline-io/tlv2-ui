    ...
    <div v-else-if="entity">
          ...
          <table class="table is-borderless property-list tl-props">
            <tr>
              <td>
                <o-tooltip
                  dashed
                  label="A globally unique identifier for this operator"
                >
                  Onestop ID
                </o-tooltip>
              </td>
            ...
            <tr>
              <td>
                <o-tooltip
                  dashed
                  multiline
                  label="Matched agencies; see 'Sources' below for full details"
                >
                  Agencies
                </o-tooltip>
              </td>
            ...
            <tr>
              <td>
                <o-tooltip
                  dashed
                  multiline
                  label="Operators and their service areas are matched against place names from the Natural Earth project"
                >
                  Locations
                </o-tooltip>
              </td>
                ...
                <ul>
                  ...
                  >
                    ...
                    <template v-if="location.adm1_name">
                      /
                      <nuxt-link
                        :to="{ name: 'operators', query: { adm1_name: location.adm1_name, adm0_name: location.adm0_name } }"
                      >
                        {{ location.adm1_name }}
                      </nuxt-link>
                    </template>
                    <template v-if="location.city_name">
                      /
                      <nuxt-link
                        :to="{ name: 'operators', query: { city_name: location.city_name, adm1_name: location.adm1_name, adm0_name: location.adm0_name } }"
                      >
                        {{ location.city_name }}
                      </nuxt-link>
                    </template>
            ...
            <tr v-if="entity && entity.tags && Object.keys(entity.tags).length > 0">
              <td>
                <o-tooltip
                  dashed
                  multiline
                  label="Links between Transitland and other catalogs and data sources on the Internet"
                >
                  ID Crosswalk
                </o-tooltip>
              </td>
              <td>
                <ul>
                  <li v-if="entity.tags.us_ntd_id">
                    US National Transit Database (NTD) ID: <code>{{ entity.tags.us_ntd_id }}</code> <a
                      target="_blank"
      ...
      >
          ...
          >
            <div class="columns">
              <div class="column is-8">
                <p>
                  This operator includes data from the <strong>{{ feedSpec.replace('_', '-') }}</strong> feed record with
                  Onestop ID of
                  <code>{{ feedOnestopId }}</code> See the feed record for Transitland's archive of fetched versions, as
                  well as URLs for accessing the feed.
                  <!-- TODO: show different text depending upon feed.spec = GTFS, GTFS-RT, or GBFS -->
                </p>
              </div>
              <div class="column is-4 has-text-right">
                <nuxt-link
                  class="button is-primary"
                  :to="{ name: 'feeds-feed', params: { feed: feedOnestopId } }"
                >
                  View Feed Record
                </nuxt-link>
              </div>
        ...
        <o-tab-item label="Source Feeds (Advanced View)">
          <tl-msg-info>
            This operator includes data from the references listed below. These references are defined in the operator's
            Atlas record, and describe the GTFS agencies that provide the routes, stops, schedules, and other information
            for this operator. If a reference to an agency cannot be resolved, this will be noted. Please see the
            <nuxt-link :to="{ name: 'documentation' }">
            ...
            >
              ...
              >
                ...
                <template v-else-if="props.row.feed_spec == 'GTFS'">
                  <o-tooltip
                    dashed
                    label="The active version of this source feed does not contain a matching agency"
                  >
                    <o-icon icon="alert" />
                  </o-tooltip>
                </template>
      ...
      <div v-if="agencyIds.length > 0">
        <a
          v-for="[, value] of Object.entries(tabIndex)"
          :key="value"
          :name="value"
        />
        <h4 class="title is-4">
