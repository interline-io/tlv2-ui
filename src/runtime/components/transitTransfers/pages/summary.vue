    ...
    >
      <template #default="{ scenario, scenarioResult }">
        ...
        <div v-else-if="scenarioResult && scenario">
          ...
          <tl-transitTransfers-time-scoring-histogram :scenario="scenario as any" :transfer-groups="scenarioResult.transferGroups" />

          <tl-transitTransfers-data-grid
  ...
  for (const r of ret) {
    if (r.trips_missed + r.trips_close + r.trips_acceptable + r.trips_unacceptable > 0) {
      r.link_to_individual_transfers = {
        routeKey: 'transitTransfers-stationKey-transfers',
