...
const appsDir = join(__dirname, '../src/runtime/apps')
...
async function generateComponentTypes () {
  ...
  // Collect all .vue files from apps
  for await (const filePath of walk(appsDir)) {
    apps.push(filePath)
  }
  ...
  // Generate type declarations for apps (TlApps prefix)
  const appTypeDeclarations = apps.map((filePath) => {
    const componentName = getComponentName(filePath, 'TlApps', true)
    const importPath = `./apps/${filePath.replace(/\\/g, '/')}`
    return `    ${componentName}: typeof import('${importPath}')['default']`
  }).join('\n')

  const typeDeclarations = [componentTypeDeclarations, controlTypeDeclarations, appTypeDeclarations].filter(Boolean).join('\n')
...
`
  ...
  console.log(`✅ Generated component types for ${components.length} components, ${controls.length} controls, and ${apps.length} apps`)
